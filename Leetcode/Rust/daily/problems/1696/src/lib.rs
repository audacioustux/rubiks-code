...
impl Solution {
    pub fn max_result(nums: Vec<i32>, k: i32) -> i32 {
        let n = nums.len();
        let mut priority_queue = BinaryHeap::from([(nums[0], 0)]);
        ...
        for i in 1..n - 1 {
            let (best, _) = priority_queue.peek().unwrap();
            priority_queue.push((best + nums[i], i as i32));

            while priority_queue.peek().unwrap().1 + k <= i as i32 {
                priority_queue.pop();
            }
        }

        nums[n - 1] + priority_queue.peek().unwrap().0
    }
}
